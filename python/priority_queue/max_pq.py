class MaxPQ():
    ...
    def __is_max_heap(self):
        ...
        if self.pq[0] != None:
            return False
        return self.__is_max_heap_ordered(1)
    
    def __is_max_heap_ordered(self,k):
        if k > self.n:
            return True
        left = 2*k
        right = 2*k + 1
        ...
        if right <= self.n and self.__less(k,right):
            return False
        return self.__is_max_heap_ordered(left) and self.__is_max_heap_ordered(right)